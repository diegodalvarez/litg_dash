import time
import numpy as np
import pandas as pd
import yfinance as yf
import datetime as dt
import streamlit as st
import plotly.express as px
import matplotlib.pyplot as plt

import statsmodels.api as sm
from statsmodels import regression

from dash_tools import DashTools

st.set_page_config(layout="wide")

#list of options
side_column_list = ["Introduction", "season 1 beta", "market panes",
                    "portfolio panes", "text panes"]

#read text file
text = pd.read_excel("text.xlsx")

#make the sidebar
side_column = st.sidebar.selectbox("options", side_column_list)

st.title("Lookbook")


if side_column == "Introduction":
    
    st.title("Introduction:")
    
    st.subheader("Goal:")
    
    with st.beta_expander(""):
        st.write(text['text'][0])
    
    st.subheader("format")
    
    with st.beta_expander(""):
        st.write(text['text'][1])
    
    
    st.subheader("layout")
    
    st.write("for best viewing experince close the sidebar")
    st.write("")
    
    st.write("A sample plot would look like")
    
    end = dt.datetime.today()
    start = dt.datetime(end.year - 5, end.month, end.day)
    
    df = yf.download("^GSPC", start, end)['Adj Close']
    st.line_chart(df)
    
    with st.beta_expander("multi pane layout"):
        
        st.write("with 2 panes on the row it looks like")
    
        two_col1, two_col2 = st.beta_columns(2)
        
        with two_col1:
            st.line_chart(df)
            
        with two_col2:
            st.line_chart(df)
            
        st.write("with 3 panes of graphs it doesn't look good")
        
        three_col1, three_col2, three_col3 = st.beta_columns(3)
        
        with three_col1:
            st.line_chart(df)
            
        with three_col2:
            st.line_chart(df)
            
        with three_col3:
            st.line_chart(df)
            
        st.write("instead we make 2 large panes for graphs and one smaller pane for text")
        
        last_price = round(df[len(df) - 1],2)
        
        left_col1, left_col2, left_col3 = st.beta_columns((1,2,2))
        
        with left_col1:
            st.markdown("<h1 style='text-align: center; color: white;'>Last SPX Close Price: ${}</h1>".format(last_price), unsafe_allow_html=True)
            st.markdown("<h1 style='text-align: center; color: white;'>Last SPX Close Price: ${}</h1>".format(last_price), unsafe_allow_html=True)
        
        with left_col2:
            st.line_chart(df)
            
        with left_col3:
            st.line_chart(df)
        
        middle_col1, middle_col2, middle_col3 = st.beta_columns((2,1,2))
        
        with middle_col1:
            st.line_chart(df)
        
        with middle_col2:
            
            st.markdown("<h1 style='text-align: center; color: white;'>Last SPX Close Price: ${}</h1>".format(last_price), unsafe_allow_html=True)
            st.markdown("<h1 style='text-align: center; color: white;'>Last SPX Close Price: ${}</h1>".format(last_price), unsafe_allow_html=True)
            
        with middle_col3:
            st.line_chart(df)
        
        right_col1, right_col2, right_col3 = st.beta_columns((2,2,1))
        
        with right_col1:
            st.line_chart(df)
        
        with right_col2:
            st.line_chart(df)
            
        with right_col3:
            st.markdown("<h1 style='text-align: center; color: white;'>Last SPX Close Price: ${}</h1>".format(last_price), unsafe_allow_html=True)
            st.markdown("<h1 style='text-align: center; color: white;'>Last SPX Close Price: ${}</h1>".format(last_price), unsafe_allow_html=True)
            
    
    with st.beta_expander("types of plots"):
        
        st.write("There are many types of plots the three main ones used are Streamlit's built-in plotting function, Plotly and Matplotlib.")
        st.write("")
        
        st.write("Streamlit's built-in plotting function looks like this. It is very similar to matplotlib and has interactable capabilites like hover")
        st.line_chart(df)
        
        st.write("Plotly is the other tool that can be used in streamlit's GUI and also allows for interactions")
        fig = px.line(df)
        st.plotly_chart(fig, use_container_width = True)
        
        st.write("You can always add in matploltlib figures is supported but almost all graphs can be made with these plotting tools")
        st.write("")
        st.write("Although the plot types don't match up. You cannot put two plots of different types on the same row")
        
        wrong_col1, wrong_col2 = st.beta_columns(2)
        
        with wrong_col1:
            
            st.write("Streamlit")
            st.line_chart(df)
            
        with wrong_col2:
            
            st.write("Plotly")
            fig_wrong = px.line(df)
            st.plotly_chart(fig_wrong)
            
        st.write("the whole row should be streamlit only or plotly only")
        
        st.write("streamlit")
        top_match_col1, top_match_col2 = st.beta_columns(2)
        
        st.write("plotly")
        bottom_match_col1, bottom_match_col2 = st.beta_columns(2)
        
        
        with top_match_col1:
            st.line_chart(df)
            
        with top_match_col2:
            st.line_chart(df)
        
        with bottom_match_col1:
            st.plotly_chart(fig)
            
        with bottom_match_col2:
            st.plotly_chart(fig)
    
    st.subheader("Looking through the lookbook and picking out panes")
    with st.beta_expander(""):
        
        st.write(text['text'][2])
    
    st.subheader("add to this project")
    st.text("")
    
    st.subheader("Use this dashboard for your personal use")
    st.write("")
    
if side_column == "season 1 beta":
    
    st.write("Simualted with random portfolio weights")
    st.write("Is not updating itself to grab live data")
    st.write("best viewed when side bar is collapsed")
    st.write("")
    
    #this makes the portfolio pie diversification
    def portfolio_pie(dashtools):
        
        #import the daily portfolio which keeps track of each value of the stock and the portfolio value
        daily_portfolio = dashtools.daily_portfolio
        
        #get the value of the portfolio into a list
        final_val = daily_portfolio['value'][len(daily_portfolio) - 1]
        
        #this is going to make our output dataframe
        pie = dashtools.weights.reset_index().drop(columns = ["share_count"])
        
        #then this is going to rename the column
        pie.columns = ["ticker"]
        
        #sort alphabetically
        pie['ticker'] = sorted(pie['ticker'])
        
        #then initialize a new column to be empty for the percetnage value
        pie['weight'] = ""
        
        #now we want to go through all of the columns except for the last one which is portfoli ovalue
        for counter, i in enumerate(daily_portfolio.columns[:-1]):
            
            #the [i][len(daily_portfolio) - 1] will get the last value of the stock in the fund then divide by the final value
            weight = daily_portfolio[i][len(daily_portfolio) - 1] / final_val
            
            #add that weight to the dataframe
            pie['weight'][counter] = round(weight, 4)
            
        #in this case we want to make the plotly object
        diversification_pie = px.pie(dashtools.weights, values = pie['weight'], names = pie['ticker'])
        
        st.subheader("Portfolio Diversification")
        st.plotly_chart(diversification_pie)
    
    #this gets the daily returns
    def daily_returns(dashtools):
        
        #this makes the header
        st.subheader("Distribution of Portfolio's Daily Returns")
        
        #this gets the percent change
        port_rets = dashtools.daily_portfolio['value'].pct_change().dropna()
        
        #this makes the distplot
        dist_plot = px.histogram(port_rets, height = 400, width = 700, nbins = 200)
        
        #this just puts titles on it
        dist_plot.update_layout(showlegend = False, xaxis_title = "Return (%)", yaxis_title = "frequency", font = dict(size = 15))
        
        #output the px object to streamlit
        st.plotly_chart(dist_plot)
    
    #portfolio statistics function
    def port_stats(dashtools):
        
        ##############################################################################
        #this is for daily returns
        ##############################################################################
        
        #lets get the last 3 days then calculate the percent change then drop values then get the last value
        daily_return = dashtools.daily_portfolio.tail(3)['value'].pct_change().dropna().tail(1)
        
        #then multiply that to get the percentage return
        daily_return = round(float(daily_return.values) *100, 2)
        
        ##############################################################################
        #this is for weekly returns
        ##############################################################################
        
        #this gest the number of days that we are looking back it is used for making a dataframe to calculate over a range
        lookback = {0:5, 1:5, 2:2, 3:3, 4:4, 5:5, 6:5}
        
        #how many days we want to go back
        cutoff = lookback[dashtools.end_date.weekday()]
        
        #then get the data and cutoff the length we want
        port_val = dashtools.daily_portfolio['value'].iloc[-cutoff:]
        
        #get first entry
        first_day = port_val.head(1).values
        
        #get last entry
        last_day = port_val.tail(1).values
        
        #do the percent change
        weekly_return = round(float(((last_day - first_day) / first_day) * 100), 2)
        
        ##############################################################################
        #ytd return
        ##############################################################################
        ytd_date = dt.datetime(dashtools.end_date.year, 1, 1)
        ytd_days = int((dashtools.end_date - ytd_date).days / 7 *5)
        ytd_return = dashtools.daily_portfolio.tail(ytd_days)['value'].iloc[[0, -1]].pct_change().dropna()
        ytd_return = round(float(ytd_return.values) * 100, 2)
        
        ##############################################################################
        #yearly returns
        ##############################################################################
        yearly_return = dashtools.daily_portfolio.tail(252).iloc[[0, -1]]['value'].pct_change().dropna()
        yearly_return = round(float(yearly_return.values) * 100, 2)
        
        ##############################################################################
        #inception
        ##############################################################################
        inception_return = dashtools.daily_portfolio['value'].iloc[[0, -1]].pct_change().dropna()
        inception_return = round(float(inception_return.values) * 100, 2)
        
        #output header and make sure font color is right
        st.markdown("<h1 style='text-align: center; color: white;'>Today's Returns: {}%</h1>".format(daily_return), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>This week's Returns: {}%</h1>".format(weekly_return), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>365 day Return: {}%</h1>".format(yearly_return), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>YTD Returns: {}%</h1>".format(ytd_return), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>Since Inception: {}%</h1>".format(inception_return), unsafe_allow_html=True)
    
    #makes the portfolio market statistics
    def port_market(dashtools):
    
        #365 day volatility
        vol = dashtools.daily_portfolio["value"].tail(252)
        vol = round(np.log(vol / vol.shift(1)).std() * 252 ** .5, 2)
        
        #get the performance of the fund
        perf = dashtools.daily_portfolio['value']
        
        #then get the benchmark
        spx = yf.download("^GSPC", perf.index[0], perf.index[-1])['Close']
        
        #get alpha and beta
        y = dashtools.daily_portfolio['value'].pct_change().dropna().iloc[:-1]
        X = spx.pct_change().dropna()
        
        #run the model
        X = sm.add_constant(X)
        model = regression.linear_model.OLS(y,X).fit()
        
        #then find alpha and beta
        alpha = round(model.params[0], 5)
        beta = round(model.params[1], 2)
        
        #now get the sharpe 
        sharpe_df = dashtools.daily_portfolio['value'].pct_change().dropna()
        ret = sharpe_df.mean()
        stdev = sharpe_df.std()
        sharpe = round(ret / stdev, 2)
        
        #output the header and check the font color
        st.markdown("<h1 style='text-align: center; color: white;'>365 day Volatility: {}</h1>".format(vol), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>Portfolio Alpha: {}</h1>".format(alpha), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>Portfolio Beta: {}</h1>".format(beta), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>Portfolio Sharpe: {}</h1>".format(sharpe), unsafe_allow_html=True)
    
    #portfolio composition tool
    def port_comp(dashtools):
    
        #then you get the monthly portfolio price
        monthly_portfolio = dashtools.make_monthly_portfolio()
        
        #put title
        st.subheader("Portfolio Value by Composition")
        
        #output it to streamlit 
        st.bar_chart(monthly_portfolio)
    
    #this makes the performance of the fund vs SPX
    def spx_perf(dashtools):
    
        #get the value of the fund
        perf = dashtools.daily_portfolio['value']
        
        #get spx values
        spx = yf.download("^GSPC", dashtools.start_date, dashtools.end_date)['Close']
        
        #get the number of (shares) for our SPX comparison
        sp_shares = int(perf[0] / spx[0])
        
        #multiply shares to our spx to get equivalent value
        bench = spx * sp_shares
        
        #merge for output
        output_df = pd.concat([perf, bench], axis = 1)
        
        #rename columns for chart
        output_df.columns = ["Port", "SPX"]
        
        st.subheader("Portfolio vs S&P 500")
        st.line_chart(output_df, width = 500, height = 300)
    
    def make_top_row(dashtools):
        
        top_col1, top_col2, top_col3 = st.beta_columns((2,2,1))
        
        with top_col1:
            portfolio_pie(dashtools)
            
        with top_col2:
            daily_returns(dashtools)
            
        with top_col3:
            port_stats(dashtools)
    
    def make_bottom_row(dashtools):
        
        bottom_col1, bottom_col2, bottom_col3 = st.beta_columns((1,2,2))
        
        with bottom_col1:
            port_market(dashtools)
            
        with bottom_col2:
            port_comp(dashtools)
            
        with bottom_col3:
            spx_perf(dashtools)
    

    dashtools = DashTools()

    #make update date
    update_date = dt.datetime.today().strftime("%a %D %I:%M %p")
    
    #show last time we updted
    st.title("Leed's Investment Trading Group Fund (last updated: {})".format(update_date))
    
    make_top_row(dashtools)
    make_bottom_row(dashtools)
    
    st.write("The information provided does not constitute as investment advice")
    st.write("Created by Diego Alvarez, not associated with Leeds Investment Trading Group Fund, Values may not be up-to-date or approximations, updates don't occur until trading day ends")
    
    #the cost basis day provided from LITG
    next_update = dt.datetime.today() + dt.timedelta(seconds = 60 * 60 * 4)
    
    #get update time
    next_update_time = next_update.strftime("%a %D %I:%M %p")
    
    #output it
    st.write("next update is scheduled for {}".format(next_update_time))
    
    #wait 30 minutes
    time.sleep(60 * 60 * 4)
    
    #then rerun the app
    st.experimental_rerun()
    
panes = pd.read_excel("panes.xlsx")

if side_column == "market panes":
    
    st.write("Simualted with random portfolio weights")
    st.write("Is not updating itself to grab live data")
    st.write("best viewed when side bar is collapsed")
    st.write("")
    
    def standard_line_chart():
    
        end_date = dt.datetime.today()
        start_date = dt.datetime(end_date.year - 5, end_date.month, end_date.day)
        spx = yf.download("^GSPC", start_date, end_date)['Adj Close']
        
        st.write("name: {}".format(panes['name'][0]))
        st.write("type: {}".format(panes['type'][0]))
        st.write("output: {}".format(panes['output'][0]))
        
        st.line_chart(spx)
    
    def ten_year_scatter():
        
        st.write("name: {}".format(panes['name'][1]))
        st.write("type: {}".format(panes['type'][1]))
        st.write("output: {}".format(panes['output'][1]))
        
        indexes = ["^GSPC", "^DJI", "^IXIC", "^RUT"]
        columns = ["SPX", "DJIA", "NASDAQ", "RUSSEL"]
        
        end = dt.datetime.today()
        start = dt.datetime(end.year - 10, end.month, end.day)
        
        df = yf.download(indexes, start, end)['Adj Close']
        df.columns = columns
        
        fig = px.scatter_matrix(df)
        st.plotly_chart(fig)
        
    def five_year_scatter():
        
        st.write("name: {}".format(panes['name'][2]))
        st.write("type: {}".format(panes['type'][2]))
        st.write("output: {}".format(panes['output'][2]))
        
        indexes = ["^GSPC", "^DJI", "^IXIC", "^RUT"]
        columns = ["SPX", "DJIA", "NASDAQ", "RUSSEL"]
        
        end = dt.datetime.today()
        start = dt.datetime(end.year - 5, end.month, end.day)
        
        df = yf.download(indexes, start, end)['Adj Close']
        df.columns = columns
        
        fig = px.scatter_matrix(df)
        st.plotly_chart(fig)
        
    def one_year_scatter():
        
        st.write("name: {}".format(panes['name'][3]))
        st.write("type: {}".format(panes['type'][3]))
        st.write("output: {}".format(panes['output'][3]))
        
        indexes = ["^GSPC", "^DJI", "^IXIC", "^RUT"]
        columns = ["SPX", "DJIA", "NASDAQ", "RUSSEL"]
        
        end = dt.datetime.today()
        start = dt.datetime(end.year - 1, end.month, end.day)
        
        df = yf.download(indexes, start, end)['Adj Close']
        df.columns = columns
        
        fig = px.scatter_matrix(df)
        st.plotly_chart(fig)
        
    def six_month_scatter():
        
        st.write("name: {}".format(panes['name'][4]))
        st.write("type: {}".format(panes['type'][4]))
        st.write("output: {}".format(panes['output'][4]))
        
        indexes = ["^GSPC", "^DJI", "^IXIC", "^RUT"]
        columns = ["SPX", "DJIA", "NASDAQ", "RUSSEL"]
        
        end = dt.datetime.today()
        start = dt.datetime(end.year, end.month - 6, end.day)
        
        df = yf.download(indexes, start, end)['Adj Close']
        df.columns = columns
        
        fig = px.scatter_matrix(df)
        st.plotly_chart(fig)
        
    ten_year_scatter()
    st.write("_______________________________________________________________")
    
    five_year_scatter()
    st.write("_______________________________________________________________")

    one_year_scatter()
    st.write("_______________________________________________________________")
    
    six_month_scatter()
    st.write("_______________________________________________________________")
    
    standard_line_chart()

if side_column == "portfolio panes":
    
    st.write("Simualted with random portfolio weights")
    st.write("Is not updating itself to grab live data")
    st.write("best viewed when side bar is collapsed")
    st.write("")
    
    dashtools = DashTools()
    
    def portfolio_line_chart(dashtools):
        
        st.write("name: {}".format(panes['name'][5]))
        st.write("type: {}".format(panes['type'][5]))
        st.write("output: {}".format(panes['output'][5]))
        
        st.line_chart(dashtools.daily_portfolio['value'])
        
    #this makes the performance of the fund vs SPX
    def spx_perf(dashtools):
        
        st.write("name: {}".format(panes['name'][6]))
        st.write("type: {}".format(panes['type'][6]))
        st.write("output: {}".format(panes['output'][6]))
    
        #get the value of the fund
        perf = dashtools.daily_portfolio['value']
        
        #get spx values
        spx = yf.download("^GSPC", dashtools.start_date, dashtools.end_date)['Close']
        
        #get the number of (shares) for our SPX comparison
        sp_shares = int(perf[0] / spx[0])
        
        #multiply shares to our spx to get equivalent value
        bench = spx * sp_shares
        
        #merge for output
        output_df = pd.concat([perf, bench], axis = 1)
        
        #rename columns for chart
        output_df.columns = ["Port", "SPX"]
        
        st.subheader("Portfolio vs S&P 500")
        st.line_chart(output_df, width = 500, height = 300)
        
    #this makes the portfolio pie diversification
    def portfolio_pie(dashtools):
        
        st.write("name: {}".format(panes['name'][7]))
        st.write("type: {}".format(panes['type'][7]))
        st.write("output: {}".format(panes['output'][7]))        
        
        #import the daily portfolio which keeps track of each value of the stock and the portfolio value
        daily_portfolio = dashtools.daily_portfolio
        
        #get the value of the portfolio into a list
        final_val = daily_portfolio['value'][len(daily_portfolio) - 1]
        
        #this is going to make our output dataframe
        pie = dashtools.weights.reset_index().drop(columns = ["share_count"])
        
        #then this is going to rename the column
        pie.columns = ["ticker"]
        
        #sort alphabetically
        pie['ticker'] = sorted(pie['ticker'])
        
        #then initialize a new column to be empty for the percetnage value
        pie['weight'] = ""
        
        #now we want to go through all of the columns except for the last one which is portfoli ovalue
        for counter, i in enumerate(daily_portfolio.columns[:-1]):
            
            #the [i][len(daily_portfolio) - 1] will get the last value of the stock in the fund then divide by the final value
            weight = daily_portfolio[i][len(daily_portfolio) - 1] / final_val
            
            #add that weight to the dataframe
            pie['weight'][counter] = round(weight, 4)
            
        #in this case we want to make the plotly object
        diversification_pie = px.pie(dashtools.weights, values = pie['weight'], names = pie['ticker'])
        
        st.subheader("Portfolio Diversification")
        st.plotly_chart(diversification_pie)
        
    #this gets the daily returns
    def daily_returns(dashtools):
        
        st.write("name: {}".format(panes['name'][8]))
        st.write("type: {}".format(panes['type'][8]))
        st.write("output: {}".format(panes['output'][8])) 
        
        #this makes the header
        st.subheader("Distribution of Portfolio's Daily Returns")
        
        #this gets the percent change
        port_rets = dashtools.daily_portfolio['value'].pct_change().dropna()
        
        #this makes the distplot
        dist_plot = px.histogram(port_rets, height = 400, width = 700, nbins = 200)
        
        #this just puts titles on it
        dist_plot.update_layout(showlegend = False, xaxis_title = "Return (%)", yaxis_title = "frequency", font = dict(size = 15))
        
        #output the px object to streamlit
        st.plotly_chart(dist_plot)
            
    #portfolio composition tool
    def port_comp(dashtools):
    
        st.write("name: {}".format(panes['name'][9]))
        st.write("type: {}".format(panes['type'][9]))
        st.write("output: {}".format(panes['output'][9])) 
    
        #then you get the monthly portfolio price
        monthly_portfolio = dashtools.make_monthly_portfolio()
        
        #put title
        st.subheader("Portfolio Value by Composition")
        
        #output it to streamlit 
        st.bar_chart(monthly_portfolio)
    
    portfolio_line_chart(dashtools)
    st.write("_______________________________________________________________")
    
    spx_perf(dashtools)
    st.write("_______________________________________________________________")
    
    portfolio_pie(dashtools)
    st.write("_______________________________________________________________")
    
    daily_returns(dashtools)
    st.write("_______________________________________________________________")
    
    port_comp(dashtools)
    st.write("_______________________________________________________________")
    
if side_column == "text panes":
    
    dashtools = DashTools()
    
    st.write("Simualted with random portfolio weights")
    st.write("Is not updating itself to grab live data")
    st.write("best viewed when side bar is collapsed")
    st.write("")
    
    #portfolio statistics function
    def port_stats(dashtools):
        
        st.write("name: {}".format(panes['name'][10]))
        st.write("type: {}".format(panes['type'][10]))
        st.write("output: {}".format(panes['output'][10])) 
        
        ##############################################################################
        #this is for daily returns
        ##############################################################################
        
        #lets get the last 3 days then calculate the percent change then drop values then get the last value
        daily_return = dashtools.daily_portfolio.tail(3)['value'].pct_change().dropna().tail(1)
        
        #then multiply that to get the percentage return
        daily_return = round(float(daily_return.values) *100, 2)
        
        ##############################################################################
        #this is for weekly returns
        ##############################################################################
        
        #this gest the number of days that we are looking back it is used for making a dataframe to calculate over a range
        lookback = {0:5, 1:5, 2:2, 3:3, 4:4, 5:5, 6:5}
        
        #how many days we want to go back
        cutoff = lookback[dashtools.end_date.weekday()]
        
        #then get the data and cutoff the length we want
        port_val = dashtools.daily_portfolio['value'].iloc[-cutoff:]
        
        #get first entry
        first_day = port_val.head(1).values
        
        #get last entry
        last_day = port_val.tail(1).values
        
        #do the percent change
        weekly_return = round(float(((last_day - first_day) / first_day) * 100), 2)
        
        ##############################################################################
        #ytd return
        ##############################################################################
        ytd_date = dt.datetime(dashtools.end_date.year, 1, 1)
        ytd_days = int((dashtools.end_date - ytd_date).days / 7 *5)
        ytd_return = dashtools.daily_portfolio.tail(ytd_days)['value'].iloc[[0, -1]].pct_change().dropna()
        ytd_return = round(float(ytd_return.values) * 100, 2)
        
        ##############################################################################
        #yearly returns
        ##############################################################################
        yearly_return = dashtools.daily_portfolio.tail(252).iloc[[0, -1]]['value'].pct_change().dropna()
        yearly_return = round(float(yearly_return.values) * 100, 2)
        
        ##############################################################################
        #inception
        ##############################################################################
        inception_return = dashtools.daily_portfolio['value'].iloc[[0, -1]].pct_change().dropna()
        inception_return = round(float(inception_return.values) * 100, 2)
        
        #output header and make sure font color is right
        st.markdown("<h1 style='text-align: center; color: white;'>Today's Returns: {}%</h1>".format(daily_return), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>This week's Returns: {}%</h1>".format(weekly_return), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>365 day Return: {}%</h1>".format(yearly_return), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>YTD Returns: {}%</h1>".format(ytd_return), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>Since Inception: {}%</h1>".format(inception_return), unsafe_allow_html=True)
    
    #makes the portfolio market statistics
    def port_market(dashtools):
    
        st.write("name: {}".format(panes['name'][11]))
        st.write("type: {}".format(panes['type'][11]))
        st.write("output: {}".format(panes['output'][11])) 
    
        #365 day volatility
        vol = dashtools.daily_portfolio["value"].tail(252)
        vol = round(np.log(vol / vol.shift(1)).std() * 252 ** .5, 2)
        
        #get the performance of the fund
        perf = dashtools.daily_portfolio['value']
        
        #then get the benchmark
        spx = yf.download("^GSPC", perf.index[0], perf.index[-1])['Close']
        
        #get alpha and beta
        y = dashtools.daily_portfolio['value'].pct_change().dropna().iloc[:-1]
        X = spx.pct_change().dropna()
        
        #run the model
        X = sm.add_constant(X)
        model = regression.linear_model.OLS(y,X).fit()
        
        #then find alpha and beta
        alpha = round(model.params[0], 5)
        beta = round(model.params[1], 2)
        
        #now get the sharpe 
        sharpe_df = dashtools.daily_portfolio['value'].pct_change().dropna()
        ret = sharpe_df.mean()
        stdev = sharpe_df.std()
        sharpe = round(ret / stdev, 2)
        
        #output the header and check the font color
        st.markdown("<h1 style='text-align: center; color: white;'>365 day Volatility: {}</h1>".format(vol), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>Portfolio Alpha: {}</h1>".format(alpha), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>Portfolio Beta: {}</h1>".format(beta), unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: white;'>Portfolio Sharpe: {}</h1>".format(sharpe), unsafe_allow_html=True)
    
    def fed_dates():
        
        st.write("name: {}".format(panes['name'][12]))
        st.write("type: {}".format(panes['type'][12]))
        st.write("output: {}".format(panes['output'][12])) 
    
        not_meeting = True
        
        #list meetings
        meetings = [dt.date(2021, 7, 28), dt.date(2021, 7, 29), 
                    dt.date(2021, 9, 22), dt.date(2021, 9, 23),
                    dt.date(2021, 11, 2), dt.date(2021, 11, 2),
                    dt.date(2021, 12, 14), dt.date(2021, 12, 15),
                    dt.date(2021, 7, 5)]
        
        day = dt.date.today()
                
        #loop through meeting days
        for i in meetings:
            
            #when it is a meeting day
            if i == day:
                
                #output that it is a meeting day
                st.markdown("<h1 style='text-align: center; color: white;'>Meeting Today</h1>", unsafe_allow_html=True)
                
                #remove that from the list
                meetings.remove(i)
                
                not_meeting = False
                
        #we want to output the days until next meeting
        if not_meeting == True:
            
            #calculate day diference
            counter = (meetings[0] - day).days
            
            #then output that
            st.markdown("<h1 style='text-align: center; color: white;'>Days Until Next Fed Meeting: {} days</h1>".format(counter), unsafe_allow_html=True)
                
        next_meeting = meetings[0].strftime("%A %d %B %Y")
        
        #then output that 
        st.markdown("<h1 style='text-align: center; color: white;'>Next Fed Meeting: {}</h1>".format(next_meeting), unsafe_allow_html=True)
        st.write("")
    
    port_stats(dashtools)
    st.write("_______________________________________________________________")
    
    port_market(dashtools)
    st.write("_______________________________________________________________")
    
    fed_dates()
    st.write("_______________________________________________________________")
        
        
        
        
        
        
        